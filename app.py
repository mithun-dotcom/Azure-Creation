...
def _connect_block(cert_path: str, cert_password: str, tenant_id: str, app_id: str) -> str:
    return f"""
...
Connect-ExchangeOnline -CertificateFilePath '{cert_path}' -CertificatePassword $securePass -AppId '{ps_escape(app_id)}' -Organization '{ps_escape(tenant_id)}' -ShowBanner:$false -ErrorAction Stop | Out-Null
"""
...
async def create_session(
    ...
    Validate credentials end-to-end by:
      ...
      2. Opening + closing an Exchange Online session (verifies cert + Exchange role)
    """
    ...
    try:
        await get_graph_token(tenantId, clientId, clientSecret)
    except HTTPException:
        os.remove(cert_path)
        raise

    test_script = (
        _connect_block(cert_path, certPassword, tenantId, clientId)
...
try {{
    $mbx = New-Mailbox -Shared -Name '{ps_escape(m["DisplayName"])}' -DisplayName '{ps_escape(m["DisplayName"])}' -Alias '{ps_escape(m["Alias"])}' -PrimarySmtpAddress $smtp -ErrorAction Stop
    Write-Host (ConvertTo-Json -Compress @{{ stage='create'; idx=$idx; smtp=$smtp; ok=$true; userId=$mbx.ExternalDirectoryObjectId; upn=$mbx.UserPrincipalName }})
...
"""
    ...
    full_script = _connect_block(cert_path, cert_pw, tenant, app_id) + actions + finalize
